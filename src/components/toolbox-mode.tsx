...
import { Ruler, Calculator, Search, ChevronRight, Activity, Percent, ArrowRight, Check, AlertTriangle, Calculator as CalcIcon, ShieldCheck } from 'lucide-react';
...
export function ToolboxMode() {
    const [activeTab, setActiveTab] = useState<'ruler' | 'calc' | 'rads'>('ruler');
                                ...
                                <div className="space-y-4">
                                    ...
                                    <div className="pb-4 border-b border-white/5">
                                        <div className="flex items-start justify-between gap-3">
                                            ...
                                            <span className="flex-shrink-0 text-[10px] px-2 py-1 rounded-full bg-emerald-500/10 text-emerald-400 border border-emerald-500/20 font-semibold">
                                                {sys.categories.length} Kategori
                                            </span>
                                        </div>
                                        <p className="text-sm text-zinc-400 mt-2 leading-relaxed">{sys.description}</p>
                                        {sys.note && (
                                            <p className="text-[11px] text-amber-400/70 mt-2 italic border-l-2 border-amber-500/30 pl-2">{sys.note}</p>
                                        )}
                                    </div>
                                    ...
                                    <div className="space-y-2">
                                        {sys.categories.map(cat => {
                                            ...
                                            return (
                                                <div
                                                    key={cat.category}
                                                    className="flex gap-3 p-3 bg-zinc-900/50 rounded-xl border border-white/5 hover:border-white/10 transition-colors"
                                                >
                                                    {/* Color bar */}
                                                    ...
                                                    <div className="flex-1 min-w-0">
                                                        <div className="flex items-center gap-2 flex-wrap mb-1">
                                                            <span className={cn("text-xs font-bold px-2 py-0.5 rounded border", colors.badge)}>
                                                                {cat.category}
                                                            </span>
                                                            <span className="text-sm font-semibold text-zinc-200">{cat.label}</span>
                                                            {cat.risk && (
                                                                <span className={cn("text-[11px] font-mono ml-auto", colors.text)}>
                                                                    {cat.risk}
                                                                </span>
                                                            )}
                                                        </div>
